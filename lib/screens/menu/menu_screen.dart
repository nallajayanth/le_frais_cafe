...
class _MenuScreenState extends State<MenuScreen> {
  ...
  // Static category names match the category field in MenuData items.
  static const List<String> _staticCategoryNames = [
    'Appetizers',
    'Chinese Starters',
    'Momos',
    'Main Course',
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await Future.wait([menu.loadMenuItems(), menu.loadCategories()]);
      if (!mounted) return;
      setState(() {
        _initialLoading = false;
        _usingApiData = menu.menuItems.isNotEmpty;
  ...
  // Items to display — API data preferred, static data as offline fallback.
  List<MenuItem> get _allItems {
    if (_initialLoading) return [];
    final provider = context.read<MenuProvider>();
    if (provider.menuItems.isNotEmpty) return List<MenuItem>.from(provider.menuItems);
    // Use static data as offline fallback when the API failed.
    return MenuData.getAllItems();
  }

  // Filter chips: (filterId, displayName). filterId null = "All".
  // When showing API data → use category ObjectIds so i.category == filterId.
  // When showing static data → use category names so i.category == filterId.
  List<(String?, String)> get _categoryOptions {
    if (_initialLoading) return [(null, 'All')];
    final provider = context.read<MenuProvider>();
    if (provider.menuItems.isNotEmpty && provider.categories.isNotEmpty) {
      // Use category NAME as filterId — the backend returns item.category as
      // a name string (e.g. "Appetizers"), so filterId must also be the name.
      return [
        (null, 'All'),
        ...provider.categories.map((c) => (c.name, c.name)),
      ];
    }
    // Static fallback — also uses category names.
    return [
      (null, 'All'),
      ..._staticCategoryNames.map((n) => (n, n)),
    ];
  }

  List<MenuItem> get _visibleItems {
    List<MenuItem> items = _allItems;

    if (_selectedCategoryId != null) {
      items = items.where((i) => i.category == _selectedCategoryId).toList();
    }
                      ...
                      errorBuilder: (context, error, stackTrace) => Container(
                        ...
                        child: const Center(
                          child: Icon(Icons.image_not_supported_outlined,
                              color: Color(0xFFB0AEAA), size: 32),
                        ),
                      ),
                    ),
                  ),
                  ...
                  Row(
                    children: [
                      const Icon(Icons.star_rounded,
                          size: 12, color: Color(0xFFC88B1A)),
      ...
      child: Column(
        ...
        children: [
          const CircularProgressIndicator(
            valueColor: AlwaysStoppedAnimation<Color>(Color(0xFF1E5C3A)),
            strokeWidth: 2.5,
          ),
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.wifi_off_rounded,
                  size: 32, color: Color(0xFFCECCC8)),
            ),
            const SizedBox(height: 16),
            const Text(
              'Could not load menu',
            ...
            Text(
              ...
              style: const TextStyle(fontSize: 13, color: Color(0xFF9A9690)),
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style:
                const TextStyle(fontSize: 13, color: Color(0xFF9A9690)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    // Keep _usingApiData in sync whenever provider rebuilds.
    if (!_initialLoading && menuProvider.menuItems.isNotEmpty && !_usingApiData) {
      // Schedule after build to avoid setState-in-build
      Future.microtask(() {
        if (mounted) {
          setState(() {
            _usingApiData = true;
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintStyle: const TextStyle(
                          color: Color(0xFFB0AEAA), fontSize: 14),
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) => _categoryChip(categories[i]),
                  ),
                ),
              ),
